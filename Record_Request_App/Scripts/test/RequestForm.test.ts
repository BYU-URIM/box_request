...
test("If input for deliveryInstructions is valid, then no error message", () => {
    requestForm.deliveryInstructions = "qwerty12345"
    expect(requestForm.errorMessage).toBeUndefined()
})

test("If text for deliveryInstructions is less than 50 than input is valid and true", () => {
    requestForm.deliveryInstructions = ""
    expect(requestForm.inputIsValid).toBeTruthy()
})

test("If text for deliveryInstructions is more than 50 then input is not valid and returns false", () => {
    requestForm.deliveryInstructions =
